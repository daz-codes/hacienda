...
const NAVIGATION_HEADER = "X-Hacienda-Navigation";
const PREFETCH_HEADER = "X-Hacienda-Prefetch";
...
export class HaciendaNavigation {
  constructor(options = {}) {
    this.targetSelector = options.target || "#hacienda-page";
    this.prefetchMode = options.prefetch === false ? false : (options.prefetch || "intent");
  ...
  start() {
    ...
    document.addEventListener("hacienda:invalidate", this.boundInvalidate);

    this.updateActiveLinks();
    requestAnimationFrame(() => {
      this.dispatch("hacienda:load", {
  ...
  stop() {
    ...
    document.removeEventListener("hacienda:invalidate", this.boundInvalidate);
  ...
  handleClick(event) {
    ...
    if (!this.dispatch("hacienda:before-navigate", detail, true)) return;
  ...
  handlePopState(event) {
    ...
    if (!this.dispatch("hacienda:before-navigate", detail, true)) {
      window.location.reload();
      return;
    }

    const scroll = event.state?.hacienda?.scroll;
  ...
  handlePrefetchIntent(event) {
    ...
    if (link.closest('[data-hacienda-prefetch="off"]')) return;
  ...
  async navigate(url, options = {}) {
    ...
    try {
      ...
      if (page.reload) throw new NavigationFallback(page.url || url, "response requested a reload");
      ...
      if (!this.dispatch("hacienda:before-morph", beforeMorph, true)) {
      ...
      this.dispatch("hacienda:navigation-error", {url: fallbackURL, error});
      this.fullLoad(fallbackURL, options.historyMode === "pop");
    } finally {
      if (sequence === this.sequence) this.setLoading(false, {url, navigationType: options.navigationType});
    }
  }
  ...
  async fetchPage(url, {prefetch = false, signal} = {}) {
    ...
    const navigation = response.headers.get(NAVIGATION_HEADER.toLowerCase());
    if (navigation === "reload") return {reload: true, url: finalURL};
    if (navigation !== "morph") throw new NavigationFallback(finalURL, "response is not navigable");
    ...
    const prefetchPolicy = response.headers.get("x-hacienda-prefetch-cache");
    return {
      url: finalURL,
      target,
      title: response.headers.get("x-hacienda-title") || parsed.title,
      cacheable: !cacheControl.includes("no-store") && prefetchPolicy !== "no-store"
    };
  }
  ...
  morph(page) {
    ...
    Idiomorph.morph(current, page.target, {
      ...
      callbacks: {
        beforeNodeMorphed: (oldNode, newNode) => {
          if (oldNode.nodeType === Node.ELEMENT_NODE && oldNode.hasAttribute("data-hacienda-permanent")) {
            return false;
          }

          if (this.heliumAttributesChanged(oldNode, newNode)) {
            reactiveReplacements.push([oldNode, newNode.cloneNode(true)]);
            return false;
          }
        }
      }
    ...
    for (const [oldNode, replacement] of reactiveReplacements) {
  ...
  heliumAttributesChanged(oldNode, newNode) {
    if (oldNode.nodeType !== Node.ELEMENT_NODE || newNode.nodeType !== Node.ELEMENT_NODE) return false;

    const signature = node => Array.from(node.attributes)
      .filter(attribute => attribute.name.startsWith("@") ||
        attribute.name.startsWith(":") ||
        attribute.name.startsWith("data-he"))
      .map(attribute => `${attribute.name}=${attribute.value}`)
      .sort()
      .join("\n");
    return signature(oldNode) !== signature(newNode);
  }
  ...
  async prefetch(url) {
    ...
    const promise = this.fetchPage(key, {prefetch: true})
      ...
      .catch(error => {
        ...
        if (!(error instanceof NavigationFallback)) console.debug("Hacienda prefetch failed", error);
        return null;
      });
  ...
  eligibleLink(node) {
    ...
    if (link.closest('[data-hacienda-navigation="off"]')) return null;
  ...
  prefetchEnabled() {
    ...
    return this.prefetchMode === "intent" &&
      !connection?.saveData &&
  ...
  updateHistory(url, mode) {
    const state = {hacienda: {url, scroll: {x: 0, y: 0}}};
  ...
  updateActiveLinks() {
    ...
    document.querySelectorAll("[data-hacienda-active-prefix]").forEach(link => {
      const prefix = link.getAttribute("data-hacienda-active-prefix") || "/";
      const active = prefix === "/" ? path === "/" : path === prefix || path.startsWith(`${prefix.replace(/\/$/, "")}/`);
  ...
  setLoading(loading, detail) {
    ...
    if (loading) {
      root.setAttribute("data-hacienda-navigating", "");
      target?.setAttribute("aria-busy", "true");
      this.dispatch("hacienda:navigation-start", detail);
    } else {
      root.removeAttribute("data-hacienda-navigating");
      target?.removeAttribute("aria-busy");
      this.dispatch("hacienda:navigation-end", detail);
    }
  }
...
const script = document.querySelector("script[data-hacienda-navigation]");
if (script) {
  const navigation = new HaciendaNavigation({
    prefetch: script.dataset.prefetch === "off" ? false : script.dataset.prefetch,
    cacheSize: script.dataset.cacheSize,
    cacheTTL: script.dataset.cacheTtl
  });
  window.HaciendaNavigation = navigation;
