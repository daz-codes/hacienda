...
module Hacienda
  class Credentials
    ...
    VERSION = "v1"
    AUTH_DATA = "Hacienda::Credentials.v1"
    ...
    # config/master.key. Refuses to run while HACIENDA_MASTER_KEY is set,
    # because the env var would keep overriding the rotated file.
    def rotate(new_key: self.class.generate_key)
      if ENV["HACIENDA_MASTER_KEY"]
        raise Error, "unset HACIENDA_MASTER_KEY before rotating; it would override the new config/master.key"
      end
    ...
    def key
      value = validate_key(ENV["HACIENDA_MASTER_KEY"] || read_master_key)
    ...
    def read_master_key
      raise Error, "missing config/master.key or HACIENDA_MASTER_KEY" unless File.file?(master_key_path)
