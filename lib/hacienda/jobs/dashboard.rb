  ...
  module Jobs
    class Dashboard
      ...
      def initialize(application:, adapter: nil, recurring_path: nil, authorized: nil)
        ...
        @adapter = adapter || Hacienda.job_adapter
      ...
      def authorized?(request)
        ...
        Hacienda.env.test? || (!Hacienda.env.production? && local_request?(request))
      end
        ...
        if dashboard_password?
          [
            401,
            {
              ...
              "www-authenticate" => %(Basic realm="Hacienda Jobs")
            },
      ...
      def dashboard_password?
        !ENV["HACIENDA_DASHBOARD_PASSWORD"].to_s.empty?
      end
      ...
      def basic_auth_authorized?(request)
        ...
        secure_compare(username, ENV.fetch("HACIENDA_DASHBOARD_USERNAME", "hacienda")) &&
          secure_compare(password, ENV.fetch("HACIENDA_DASHBOARD_PASSWORD"))
      end
