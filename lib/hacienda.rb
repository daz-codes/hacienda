...
require_relative "hacienda/version"
require_relative "hacienda/environment"
require_relative "hacienda/logger"
require_relative "hacienda/response"
require_relative "hacienda/actions"
require_relative "hacienda/html"
require_relative "hacienda/template"
require_relative "hacienda/flash"
require_relative "hacienda/validations"
require_relative "hacienda/attributes"
require_relative "hacienda/store"
require_relative "hacienda/cache"
require_relative "hacienda/storage"
require_relative "hacienda/navigation"
require_relative "hacienda/sqlite"
require_relative "hacienda/migrations"
require_relative "hacienda/session_store"
require_relative "hacienda/credentials"
require_relative "hacienda/durable_queue"
require_relative "hacienda/jobs/adapter"
require_relative "hacienda/jobs"
require_relative "hacienda/jobs/benchmark"
require_relative "hacienda/jobs/dashboard"
require_relative "hacienda/jobs/outbox"
require_relative "hacienda/mailer"
require_relative "hacienda/mailer/inbox"
require_relative "hacienda/signed_token"
require_relative "hacienda/events"
require_relative "hacienda/transaction"
require_relative "hacienda/context"
require_relative "hacienda/params"
require_relative "hacienda/route"
require_relative "hacienda/routes"
require_relative "hacienda/assets"
require_relative "hacienda/renderer"
require_relative "hacienda/errors"
require_relative "hacienda/middleware/request_limits"
require_relative "hacienda/middleware/pending_migrations"
require_relative "hacienda/middleware/csrf"
require_relative "hacienda/middleware/request_logger"
require_relative "hacienda/middleware/host_authorization"
require_relative "hacienda/middleware/security_headers"
require_relative "hacienda/middleware/rate_limiter"
require_relative "hacienda/middleware/storage_files"
require_relative "hacienda/application"
...
module Hacienda
  class << self
    ...
    def credentials(root: @root)
      raise Error, "Hacienda.root is not configured" unless root
    ...
    def signed_token_secret
      ENV["HACIENDA_SECRET_KEY_BASE"] ||
        (File.file?(File.join(@root.to_s, "config", "credentials.yml.enc")) && credentials.dig(:hacienda, :secret_key_base)) ||
        development_signed_token_secret
    end

    # Previous secrets stay valid for verification, so a rotated
    # HACIENDA_SECRET_KEY_BASE doesn't invalidate outstanding tokens.
    # Comma-separated in the env var, an array in credentials.
    def signed_token_old_secrets
      from_env = ENV["HACIENDA_SECRET_KEY_BASE_OLD"].to_s.split(",").map(&:strip)
      from_credentials = if File.file?(File.join(@root.to_s, "config", "credentials.yml.enc"))
        Array(credentials.dig(:hacienda, :old_secret_key_bases))
      else
        []
      end
    ...
    def development_signed_token_secret
      raise Error, "HACIENDA_SECRET_KEY_BASE or credentials.hacienda.secret_key_base is required in production" if env.production?
    ...
    def env
      @env ||= Environment.new(ENV["HACIENDA_ENV"] || ENV["RACK_ENV"] || "development")
    end
    ...
    def credentials_app_url
      ...
      credentials.dig(:hacienda, :app_url)
    end

    def development_app_url
      raise Error, "HACIENDA_APP_URL, APP_URL, or credentials.hacienda.app_url is required in production" if env.production?
    ...
    def normalize_app_url(url)
      text = url.to_s.strip
      raise Error, "Hacienda app URL is required" if text.empty?

      uri = URI(text)
      unless uri.is_a?(URI::HTTP) && uri.host
        raise Error, "Hacienda app URL must be an absolute http:// or https:// URL"
      end
