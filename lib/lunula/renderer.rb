...
module Lunula
  class Renderer
    ...
    def initialize(root:, layout: "application", title: "Lunula")
      @root = root
      @layout = layout
      @title = title
    end
    ...
    class ViewContext
      ...
      def initialize(renderer, domain, locals, title: "Lunula")
        @renderer = renderer
        @domain = domain
        @locals = locals
      ...
      def navigation_page(content, context:)
        ...
        attributes = configured.merge(id: "morpheus-page", "data-morpheus-page": true)
