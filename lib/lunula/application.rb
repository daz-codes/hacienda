...
module Lunula
  class Application
    ...
    def transaction(**options)
      unless database
        raise Error, "database is not configured; pass database: DB to Lunula::Application.new"
      end
      ...
      database.transaction(**options) do
        yield Transaction.new(
          ...
          events:,
          outbox:,
          job_adapter: Lunula.job_adapter,
          job_outbox:
        )
      end
    end
    ...
    def action_set_class(route)
      ...
      unless action_set.is_a?(Class) && action_set < Actions
        raise Error, "#{route.action_set_name} must inherit from Lunula::Actions"
      end
      action_set
    ...
    def load_action_sets
      action_set_files.each do |file|
        ...
        unless action_set.is_a?(Class) && action_set < Actions
          raise Error, "#{name} must inherit from Lunula::Actions"
        end
        action_set.validate!
      rescue NameError => error
        raise unless missing_constant?(error, name)

        raise Error, "#{file} must define #{name} < Lunula::Actions"
      end
    end
    ...
    def finish_rendered(route, context, view:, locals:, status:, layout: UNDEFINED, navigable: true)
      ...
      if navigation.enabled?
        headers["vary"] = "X-Morpheus-Navigation"
        headers["x-morpheus-title"] = header_value(rendered.title) if morph
        headers["x-morpheus-navigation"] = morph ? "morph" : "reload" if context.navigation_request?
        if context.prefetch?
          headers["x-morpheus-prefetch-cache"] = context.flash.any? ? "no-store" : "store"
        end
      end
    ...
    def finish_explicit_response(result, context)
      ...
      headers["x-morpheus-navigation"] = "reload" if context.navigation_reload?
    ...
    def render_error_page(status, env: nil, **locals)
      ...
      Lunula.logger.error("failed to render #{status} error page: #{error.class}: #{error.message}")
      nil
    end
