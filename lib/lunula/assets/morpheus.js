...
const NAVIGATION_HEADER = "X-Morpheus-Navigation";
const PREFETCH_HEADER = "X-Morpheus-Prefetch";
...
export class Morpheus {
  constructor(options = {}) {
    this.targetSelector = options.target || "#morpheus-page";
  ...
  start() {
    ...
    document.addEventListener("morpheus:invalidate", this.boundInvalidate);

    this.updateActiveLinks();
    requestAnimationFrame(() => {
      this.dispatch("morpheus:load", {
  ...
  stop() {
    ...
    document.removeEventListener("morpheus:invalidate", this.boundInvalidate);
  ...
  handleClick(event) {
    ...
    if (!this.dispatch("morpheus:before-navigate", detail, true)) return;
  ...
  handlePopState(event) {
    ...
    if (!this.dispatch("morpheus:before-navigate", detail, true)) {
      window.location.reload();
      return;
    }

    const scroll = event.state?.morpheus?.scroll;
  ...
  handlePrefetchIntent(event) {
    ...
    if (link.closest('[data-morpheus-prefetch="off"]')) return;
  ...
  async navigate(url, options = {}) {
    ...
    try {
      ...
      if (!this.dispatch("morpheus:before-morph", beforeMorph, true)) {
      ...
      this.dispatch("morpheus:navigation-error", {url: fallbackURL, error});
  ...
  async fetchPage(url, {prefetch = false, signal} = {}) {
    ...
    const prefetchPolicy = response.headers.get("x-morpheus-prefetch-cache");
    return {
      url: finalURL,
      target,
      title: response.headers.get("x-morpheus-title") || parsed.title,
    ...
    Idiomorph.morph(current, page.target, {
      ...
      callbacks: {
        beforeNodeMorphed: (oldNode, newNode) => {
          if (oldNode.nodeType === Node.ELEMENT_NODE && oldNode.hasAttribute("data-morpheus-permanent")) {
            return false;
          }
  ...
  async prefetch(url) {
    ...
    const promise = this.fetchPage(key, {prefetch: true})
      ...
      .catch(error => {
        ...
        if (!(error instanceof NavigationFallback)) console.debug("Morpheus prefetch failed", error);
        return null;
      });
  ...
  eligibleLink(node) {
    ...
    if (link.closest('[data-morpheus="off"]')) return null;
  ...
  updateHistory(url, mode) {
    const state = {morpheus: {url, scroll: {x: 0, y: 0}}};
  ...
  updateActiveLinks() {
    ...
    document.querySelectorAll("[data-morpheus-active-prefix]").forEach(link => {
  ...
  setLoading(loading, detail) {
    ...
    if (loading) {
      root.setAttribute("data-morpheus-navigating", "");
      target?.setAttribute("aria-busy", "true");
      this.dispatch("morpheus:navigation-start", detail);
    } else {
      root.removeAttribute("data-morpheus-navigating");
      target?.removeAttribute("aria-busy");
      this.dispatch("morpheus:navigation-end", detail);
    }
  }
...
const script = document.querySelector("script[data-morpheus]");
if (script) {
