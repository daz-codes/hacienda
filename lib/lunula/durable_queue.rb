    ...
    class Queue
      ...
      def expire_exhausted_leases(at:)
        ...
        dataset
          ...
          .update({
            ...
            locked_by: nil,
            failed_at: at,
            last_error: "Lunula::Durable::LeaseExpired: worker stopped before completing its final attempt",
      ...
      def queue_error(error)
        ...
        if error.message.match?(/no such table|does not exist/i)
          "durable queue table #{table.inspect} is missing; run luna db:migrate"
        else
          error.message
        end
      end
