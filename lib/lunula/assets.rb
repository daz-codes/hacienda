...
module Lunula
  module Assets
    ...
    def precompile(root: Lunula.root)
      Compiler.new(root: required_root(root)).compile
    end

    def clobber(root: Lunula.root)
    ...
    def path(source, root: Lunula.root, environment: Lunula.env)
    ...
    def rack_options(root: Lunula.root)
    ...
    def manifest(root: Lunula.root, required: true)
      read_manifest(File.join(required_root(root), "public", "assets"), required:)
    end

    def required_root(root)
      raise Error, "Lunula.root is not configured" if root.to_s.empty?
    ...
    def read_manifest(assets_root, required:)
      ...
      unless File.file?(path)
        ...
        raise Error, "asset manifest not found at #{path}; run `bundle exec luna assets:precompile`"
      end
