...
class SessionStoreTest < Minitest::Test
  def setup
    ...
    Lunula::SessionStore.create_table(@database)
  end
  ...
  def test_persists_session_data_server_side
    ...
    assert_equal 1, @database[:lunula_sessions].count
  ...
  def test_revoke_deletes_a_session_by_public_cookie_id
    ...
    request = Rack::MockRequest.new(app)
    first = request.get("/")
    public_id = Rack::Utils.parse_cookies_header(first["set-cookie"]).fetch("lunula.session")

    assert Lunula::SessionStore.new(->(_env) { [204, {}, []] }, database: @database).revoke(public_id)
    assert_equal 0, @database[:lunula_sessions].count
  end
  ...
  def test_expired_sessions_are_ignored_and_can_be_pruned
    ...
    assert_equal 2, @database[:lunula_sessions].count
