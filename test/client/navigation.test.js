...
import {HaciendaNavigation} from "../../lib/hacienda/assets/hacienda-navigation.js";

const page = (name, options = {}) => `
  <div id="hacienda-page" data-hacienda-page>
    <a id="destination" href="${name === "home" ? "/next" : "/"}">Navigate</a>
    <p>${name}</p>
    <aside id="permanent" data-hacienda-permanent>${options.permanent || "keep"}</aside>
...
const response = (name, options = {}) => new Response(page(name, options), {
  ...
  headers: {
    ...
    "x-hacienda-navigation": options.navigation || "morph",
    "x-hacienda-title": name === "next" ? "Next" : "Home",
    "x-hacienda-prefetch-cache": options.cache || "store"
  }
});

const settle = (milliseconds = 0) => new Promise(resolve => setTimeout(resolve, milliseconds));

describe("HaciendaNavigation", () => {
  let navigation;
  ...
  beforeEach(() => {
    ...
    navigation = new HaciendaNavigation({cacheSize: 2, cacheTTL: 15});
  });
  ...
  test("prefetches on intent, morphs one target, and restores history", async () => {
    ...
    observer.observe(document.querySelector("#hacienda-page"), {childList: true, subtree: true});
    const loads = [];
    document.addEventListener("hacienda:load", event => loads.push({detail: event.detail, observerFinished}), {once: false});
    ...
    expect(requests[0].options.headers["X-Hacienda-Prefetch"]).toBe("true");
    ...
    expect(document.activeElement).toBe(document.querySelector("#hacienda-page"));
    expect(loads.at(-1).detail.prefetched).toBe(true);
    expect(loads.at(-1).observerFinished).toBe(true);

    history.replaceState({hacienda: {scroll: {x: 4, y: 12}}}, "", "/");
    window.dispatchEvent(new PopStateEvent("popstate", {state: history.state}));
    await settle(30);

    expect(requests).toHaveLength(2);
    expect(document.title).toBe("Home");
    expect(document.querySelector("#hacienda-page").textContent).toContain("home");
  ...
  test("bounds and invalidates the prefetch cache", async () => {
    ...
    document.dispatchEvent(new CustomEvent("hacienda:invalidate"));
    expect(navigation.cache.size).toBe(2);
  ...
  test("falls back for non-navigable responses and respects link opt-outs", async () => {
    document.body.innerHTML = `${page("home")}<div data-hacienda-navigation="off"><a id="native" href="/native">Native</a></div>`;
