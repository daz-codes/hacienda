...
class CredentialsTest < Minitest::Test
  def setup
    @directory = Dir.mktmpdir("lunula-credentials")
    @credentials = Lunula::Credentials.new(root: @directory).ensure_files
  ...
  def test_rotate_re_encrypts_with_a_new_master_key
    ...
    assert_equal "secret", Lunula::Credentials.new(root: @directory).dig(:mail, :password)
  end
  ...
  def test_rotate_accepts_an_explicit_key
    new_key = Lunula::Credentials.generate_key
  ...
  def test_rotate_validates_the_new_key
    assert_raises(Lunula::Credentials::Error) { @credentials.rotate(new_key: "not-hex") }
  end

  def test_rotate_refuses_while_the_env_master_key_is_set
    ENV["LUNULA_MASTER_KEY"] = Lunula::Credentials.generate_key

    error = assert_raises(Lunula::Credentials::Error) { @credentials.rotate }

    assert_includes error.message, "LUNULA_MASTER_KEY"
  ensure
