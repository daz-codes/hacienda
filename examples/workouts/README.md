...
A Lunula version of the Volt workout generator. It demonstrates a substantial
...
- Sequel persistence with explicit JSON load/dump coercions in `Lunula::Store`;
...
- Morpheus for prefetched, Idiomorph-powered GET transitions.
...
as `openai.api_key` in Lunula encrypted credentials instead.
...
The repository uses `dataset` for custom ordering and configures the two
